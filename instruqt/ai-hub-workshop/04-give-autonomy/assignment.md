...
So, to begin, create a new file in `src/Warehouse/AI` called `OrderTools.cls`. Then add the following:

- A property, `DraftOrder`, which should be an instance of `Warehouse.Data.DraftOrder`. This class is available if you would like to take a look. 
...
<details><summary>Full Class</summary>
...
Open the `src/Warehouse/AI/ToolSet.cls` file and add the following line to the xml definition: 

```xml
<Include Class="Warehouse.AI.OrderTools"/>
```
